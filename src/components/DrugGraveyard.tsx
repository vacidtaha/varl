...
export default function DrugGraveyard() {
  ...
  if (!mounted) {
    return (
      <div className="mx-auto w-[1300px] max-w-full px-5 lg:px-0">
  ...
  return (
    <div className="mx-auto w-[1300px] max-w-full px-5 lg:px-0">
      {/* Header */}
      <div className="mb-8 flex flex-col gap-4 lg:mb-12 lg:flex-row lg:items-end lg:justify-between lg:gap-0">
        <div>
          <span className="text-[10px] tracking-widest text-gray-400 lg:text-xs" style={{ fontWeight: 500 }}>THE DRUG GRAVEYARD</span>
          <h3 className="mt-2 text-2xl tracking-tight text-gray-900 lg:mt-3 lg:text-4xl dark:text-gray-100" style={{ fontWeight: 500 }}>
            {failedDrugs.length} Compounds.<br className="lg:hidden" /> Zero Patients Helped.
          </h3>
        </div>
        <div className="lg:shrink-0 lg:text-right">
          <span className="text-3xl font-semibold tracking-tight text-red-500 lg:text-5xl">${(totalCostM / 1000).toFixed(1)}B</span>
          <p className="mt-1 text-xs text-gray-400 lg:text-sm">Total investment lost</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-1">
        {failedDrugs.map((drug, i) => {
          ...
          return (
            ...
            >
              <span
                className="inline-block cursor-default whitespace-nowrap rounded px-1.5 py-0.5 font-mono text-[9px] transition-all duration-1000 lg:px-2 lg:py-1 lg:text-[11px]"
                style={{
              ...
              <div className="pointer-events-none absolute -top-14 left-1/2 z-50 hidden -translate-x-1/2 scale-95 rounded-lg bg-gray-100 px-4 py-2.5 text-left opacity-0 shadow-lg transition-all duration-200 group-hover:scale-100 group-hover:opacity-100 lg:block dark:bg-neutral-800">
      ...
      {/* Bottom */}
      <div className="mt-6 flex items-start gap-3 lg:mt-10 lg:items-center">
        <div className="mt-1 h-2 w-2 shrink-0 animate-pulse rounded-full bg-red-500 lg:mt-0"></div>
        <p className="text-xs text-gray-500 lg:text-sm dark:text-gray-400">
