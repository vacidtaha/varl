...
export default function FoodStories() {
  ...
  return (
    <div className="mx-auto w-[1500px] max-w-full">
      {/* Desktop: Image with side buttons */}
      <div className="relative hidden items-center gap-6 px-8 lg:flex">
        <button
        ...
        <div className="relative h-[720px] flex-1 overflow-hidden rounded-2xl">
          <Image
            src={story.img}
            alt={story.name}
            fill
      ...
      {/* Mobile: Image without side buttons */}
      <div className="px-4 lg:hidden">
        <div className="relative aspect-[4/3] w-full overflow-hidden rounded-2xl">
          ...
          <div className="absolute bottom-4 left-5">
            <span className="text-sm text-white" style={{ fontWeight: 500 }}>{story.name}</span>
            <span className="ml-2 text-xs text-white/60">{story.location}</span>
          </div>
        </div>
        ...
        <div className="mt-4 flex items-center justify-center gap-3">
          <button
            onClick={() => setActive(active === 0 ? stories.length - 1 : active - 1)}
            className="flex h-10 w-10 items-center justify-center rounded-full bg-gray-200 text-gray-400 transition-colors hover:bg-gray-300 hover:text-gray-700 dark:bg-neutral-800 dark:text-gray-500 dark:hover:bg-neutral-700 dark:hover:text-white"
          >
            ...
            {stories.map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  i === active ? "w-5 bg-green-600" : "w-1.5 bg-gray-300 dark:bg-neutral-700"
                }`}
              />
            ))}
          </div>
      ...
      {/* Desktop dots */}
      <div className="mt-6 hidden justify-center gap-1.5 lg:flex">
        {stories.map((_, i) => (
          <button
            key={i}
            onClick={() => setActive(i)}
      ...
      {/* Story text */}
      <div className="mx-auto mt-4 max-w-2xl px-5 text-center lg:px-0">
